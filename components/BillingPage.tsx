...
export const shareInvoiceOnWhatsApp = (
    ...
    if (lang === 'ar') {
        message = `*فاتورة طلب رقم ${order.localOrderId}*\n\n` +
            `مرحباً ${client.name}،\n` +
            `إليك تفاصيل فاتورتك من ${companyInfo.name}:\n\n` +
            `📦 *المتجر:* ${store?.name || 'غير محدد'}\n` +
            `⚖️ *الوزن:* ${order.weight || 0} kg\n` +
            `💰 *قيمة المنتج:* ${productTotal.toLocaleString()} MRU\n` +
            `🚚 *الشحن:* ${shipping.toLocaleString()} MRU\n` +
            (delivery > 0 ? `🛵 *التوصيل:* ${delivery.toLocaleString()} MRU\n` : '') +
            `-------------------------\n` +
            `💵 *الإجمالي:* ${totalDue.toLocaleString()} MRU\n` +
            `✅ *المدفوع:* ${paid.toLocaleString()} MRU\n` +
            `❗️ *المتبقي:* ${remaining.toLocaleString()} MRU\n\n` +
            `شكراً لثقتكم بنا!`;
    } else {
        message = `*Invoice Order #${order.localOrderId}*\n\n` +
            `Hello ${client.name},\n` +
            `Here are your invoice details from ${companyInfo.name}:\n\n` +
            `📦 *Store:* ${store?.name || 'N/A'}\n` +
            `⚖️ *Weight:* ${order.weight || 0} kg\n` +
            `💰 *Product:* ${productTotal.toLocaleString()} MRU\n` +
            `🚚 *Shipping:* ${shipping.toLocaleString()} MRU\n` +
            (delivery > 0 ? `🛵 *Delivery:* ${delivery.toLocaleString()} MRU\n` : '') +
            `-------------------------\n` +
            `💵 *Total:* ${totalDue.toLocaleString()} MRU\n` +
            `✅ *Paid:* ${paid.toLocaleString()} MRU\n` +
            `❗️ *Balance:* ${remaining.toLocaleString()} MRU\n\n` +
