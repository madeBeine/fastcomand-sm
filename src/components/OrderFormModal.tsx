...
import { Save, X, Plus, Trash2, UploadCloud, Loader2, Link, Ship, Plane, Truck, DollarSign, Image as ImageIcon, User, ShoppingBag, CheckCircle2, Calculator, Receipt, ChevronDown, Upload, Search, Wallet, Sparkles, Wand2, Calendar, Tag, Clock, FileText, AlertCircle } from 'lucide-react';
...
function SearchableSelect<T extends { id: string }>({ options, value, placeholder, onChange, getDisplayValue, renderOption, valueField, error, icon, onSearch }: SearchableSelectProps<T>) {
    ...
    const filteredOptions = useMemo(() => {
        ...
        const isVeryShort = query.length < 2;
        ...
        return options
            .map(option => {
                ...
                if (name === normalizedSearch || (phoneSearch.length > 3 && phone === phoneSearch)) {
                    score += 10000;
                }
                else if (name.startsWith(normalizedSearch) || (phoneSearch.length > 2 && phone.startsWith(phoneSearch))) {
                    score += 5000;
                }
                else if (name.split(' ').some(word => word === normalizedSearch)) {
                    score += 2000;
                }
                else if (!isVeryShort && (name.includes(normalizedSearch) || (phoneSearch.length > 3 && phone.includes(phoneSearch)))) {
                    score += 100;
                }
    ...
    return (
        <div className="relative" ref={wrapperRef}>
                ...
                <ul className="absolute z-[100] w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl max-h-64 overflow-y-auto animate-in fade-in slide-in-from-top-1 duration-200 custom-scrollbar">
                    {filteredOptions.length > 0 ? (
                        filteredOptions.map(option => (
                            <li key={option.id} onClick={() => selectOption(option)} className="px-4 py-3 cursor-pointer hover:bg-primary/5 dark:hover:bg-primary/10 border-b dark:border-gray-700 last:border-0 transition-colors">
                                {renderOption(option)}
                            </li>
                        ))
                    ) : (
            ...
            {error && <p className="text-red-500 text-[10px] mt-1 font-black">{error}</p>}
        </div>
    );
}
...
const OrderFormModal: React.FC<OrderFormModalProps> = ({ isOpen, onClose, onSave, order, clients, stores, currencies, commissionRate, isSaving = false, orders = [], settings, shippingCompanies = [], paymentMethods = [], onClientSearch }) => {
    ...
    const getInitialFormData = (): Partial<Order> => {
        if (order) {
            return {
                ...
                receiptImages: order.receiptImages || [],
                paymentMethod: order.paymentMethod || (paymentMethods?.[0]?.name || 'Cash'),
    ...
    const financials = useMemo(() => {
        ...
        if (formData.commissionType === 'percentage') {
            calculatedCommission = Math.max(totalMRU * ((formData.commissionRate || 0) / 100), minCommission);
        } else {
    ...
    }, [formData.price, formData.currency, formData.commissionType, formData.commissionRate, formData.commission, formData.amountPaid, currencies, settings]);
    ...
    const handleMagicFill = async () => {
        ...
        if (!link) return alert("يرجى إدخال رابط منتج أولاً للمسح الذكي.");
        setIsAILoading(true);
        try {
        ...
        } catch (e) { alert("فشل المساعد الذكي في تحليل الرابط."); } finally { setIsAILoading(false); }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const finalData = { ...formData, commission: financials.calculatedCommission, priceInMRU: financials.totalMRU, productImages: productImagePreviews, receiptImages: receiptImagePreviews, receiptImage: receiptImagePreviews?.[0] || null };
            await onSave(finalData as Order);
            onClose();
        } finally { setLocalIsSaving(false); }
    };

    const inputClass = "w-full h-[46px] px-4 border rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary/50 text-sm border-gray-300 dark:border-gray-600 font-medium transition-all";
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 p-0 md:p-4" onClick={onClose}>
            <div className="bg-white dark:bg-gray-900 w-full h-full md:h-[95vh] md:max-w-5xl md:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="col-span-2">
                                        ...
                                        <input type="text" name="localOrderId" value={formData.localOrderId || ''} onChange={handleInputChange} className={`${inputClass} font-mono font-bold text-primary`} placeholder="FCD-1234" />
                                    </div>
                                    <div>
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="number" name="quantity" value={formData.quantity} onChange={handleInputChange} className={inputClass} min="1" />
                                        </div>
                                        <div>
                                            <label className={labelClass}>{t('totalPrice')}*</label>
                                            <div className="flex relative">
                                                <input type="number" name="price" value={formData.price ?? ''} onChange={handleInputChange} className={`${inputClass} text-lg font-black pr-24`} placeholder="0.00" />
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="col-span-2">
                                            ...
                                            <div className="flex gap-2">
                                                <input type="number" name="amountPaid" value={formData.amountPaid ?? ''} onChange={handleInputChange} className={`${inputClass} font-black text-green-600`} placeholder="0.00" />
                                                ...
                                                <div className={`mt-2 flex items-center gap-2 px-3 py-2 rounded-lg border animate-in slide-in-from-top-1 duration-200 ${financials.remaining > 0 ? 'bg-orange-50 border-orange-100 dark:bg-orange-900/10 dark:border-orange-800 text-orange-700 dark:text-orange-400' : 'bg-green-50 border-green-100 dark:bg-green-900/10 dark:border-green-800 text-green-700 dark:text-green-400'}`}>
                                                    ...
                                                    <span className="text-xs font-bold">
                                                        {financials.remaining > 0 
                                                            ? `المتبقي: ${financials.remaining.toLocaleString()} MRU` 
                            ...
                            <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div><label className={labelClass}>{t('origin')}*</label><select name="originCenter" value={formData.originCenter || ''} onChange={handleInputChange} className={inputClass}>{availableZones.map(z => <option key={z} value={z}>{z}</option>)}</select></div>
                ...
                <div className="p-4 md:p-6 border-t dark:border-gray-800 flex justify-end gap-3 bg-white dark:bg-gray-900">
                    ...
                    <button onClick={handleSubmit} disabled={localIsSaving || isProcessingImages} className="px-10 py-3 bg-primary text-white rounded-xl font-black shadow-xl flex items-center gap-2 disabled:opacity-50 transition-all active:scale-95">
