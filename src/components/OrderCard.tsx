        ...
        <>
            ...
            `}>
                ...
                <div className="px-6 py-2 space-y-4">
                    {/* Client Section (Interactive) */}
                    <div 
                        onClick={() => client && onClientClick && onClientClick(client)}
                        className={`
                            flex items-center gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-[1.8rem] border border-slate-100 dark:border-slate-800 transition-all
                            ${client ? 'cursor-pointer hover:border-primary/50 hover:bg-white dark:hover:bg-slate-800 hover:shadow-md group/client' : 'bg-gray-100/50 cursor-help border-dashed'}
                        `}
                    >
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {/* Quantity Badge */}
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-purple-50 dark:bg-purple-900/30 text-[10px] font-black text-purple-600 dark:text-purple-400">
                            <Package size={14}/> العدد: {order.quantity}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            {order.trackingNumber && (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-[11px] font-bold font-mono text-gray-700 dark:text-gray-300 truncate">{order.trackingNumber}</span>
                ...
                <div className="mt-auto p-5 space-y-3">
                    ...
                    {!isCompleted && order.status === OrderStatus.ORDERED ? (
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => onSendNotification(order)} className="flex-1 py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-2xl font-black shadow-lg shadow-emerald-500/20 transition-all flex items-center justify-center gap-3 active:scale-95 group/btn">
                                <MessageCircle size={20}/> {order.whatsappNotificationSent ? 'إعادة التنبيه' : 'تنبيه العميل'}
                            </button>
                            <button 
                                onClick={() => onUpdatePayment && onUpdatePayment(order)}
                                className="w-14 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl flex items-center justify-center shadow-lg transition-all active:scale-95"
                                title="تحديث المدفوعات / التسليم"
                            >
                                <Wallet size={20}/>
                            </button>
                        </div>
                    ) : isCompleted ? (
                        <div className="w-full py-3 bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded-2xl font-black text-center flex items-center justify-center gap-2">
                            <CheckCircle2 size={18}/> طلب مكتمل (مؤرشف)
                        </div>
                    ) : null}
                        ...
                        <div className="flex gap-2">
                            {validLinks.length > 0 && (
                                <button 
                                    ...
                                    className="p-3 bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 rounded-2xl hover:bg-blue-600 hover:text-white transition-all shadow-sm relative"
                                    title="روابط المنتجات"
                                >
                                    <ExternalLink size={18}/>
                                    {validLinks.length > 1 && (
                                        <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full shadow-sm">{validLinks.length}</span>
                                    )}
                                </button>
                            )}
